...
#define immJ()                                                          \
  do                                                                    \
  {                                                                     \
    *imm = (BITS(i, 19, 12) << 12) | (SEXT(BITS(i, 31, 31), 1) << 20) | \
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhsu, R,
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge, B,
          s->dnpc = ((int32_t)src1 >= (int32_t)src2) ? (s->pc + imm) : s->dnpc);
  ...
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub, R,
          R(rd) = src1 - src2);
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mui, R,
          R(rd) = src1 * src2);
